...
void	sort_3(t_stack *a_stack)
{
	...
	if (is_sorted(a_stack->arr, a_stack->size, a_stack->top))
		return ;
	if (arr[a_stack->top + 2] > arr[a_stack->top] && arr[a_stack->top]
		> arr[a_stack->top + 1])
		swap(a_stack, false);
	else if (arr[a_stack->top] > arr[a_stack->top + 1] && arr[a_stack->top + 1]
		> arr[a_stack->top + 2])
	{
		swap(a_stack, false);
		reverse_rotate(a_stack, false);
	}
	else if (arr[a_stack->top] > arr[a_stack->top + 2] && arr[a_stack->top + 2]
		> arr[a_stack->top + 1])
		rotate(a_stack, false);
	else if (arr[a_stack->top + 1] > arr[a_stack->top + 2] && arr[a_stack->top
			+ 2] > arr[a_stack->top])
	{
		swap(a_stack, false);
		rotate(a_stack, false);
